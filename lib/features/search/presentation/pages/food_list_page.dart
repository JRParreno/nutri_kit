...
class FoodListPage extends StatefulWidget {
  const FoodListPage({super.key, required this.keyword});
...
class _FoodListPageState extends State<FoodListPage> {
  ...
  void initState() {
    super.initState();
    context.read<SearchFoodBloc>().add(GetSearchFoodEvent(widget.keyword));
  ...
  void handleEventScrollListener() {
    controller.addListener(() {
      if (controller.position.pixels > (controller.position.pixels * 0.75)) {
        context
            .read<SearchFoodBloc>()
            .add(PaginateSearchFoodEvent(widget.keyword));
      }
    });
  }
}
